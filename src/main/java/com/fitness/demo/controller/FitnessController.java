...
public class FitnessController {
    private static final Logger log = LoggerFactory.getLogger(FitnessController.class);
    ...
    public ResponseEntity<ExerciseDetailsDao> addExerciseDetails(@RequestBody ExerciseDetailsDto exerciseDetailsDto) {
        ...
        // Status code 201 signifies that request has succeeded and git chas led to the
    ...
    public ResponseEntity<WorkoutDao> updateWorkout(@PathVariable Long workoutId,
            @RequestBody WorkoutDao updatedWorkout) {
        WorkoutDao updated = fitnessService.updateWorkout(workoutId, updatedWorkout);
        return ResponseEntity.ok(updated);
    }

    @GetMapping("workout/get/")
    public ResponseEntity<List<WorkoutDao>> getWorkout(
            @RequestParam("d1") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime date1,
